<template>
  <div class="my-box">
    <van-cell v-if="!isLogin" class="info" :title="userInfo.name" is-link :to="userInfo.to">
      <template #icon>
        <van-icon :name="userInfo.icon" class="user-icon" />
      </template>
    </van-cell>
    <van-cell v-else class="info" :title="newUserInfo.name" is-link :to="newUserInfo.to">
      <template #icon>
        <van-icon :name="newUserInfo.icon" class="user-icon" />
      </template>
    </van-cell>
    ...
    <div class="focus-box">
      <div class="focus-item" v-for="(item, index) in focusList" :key="index">
    ...
    >
      <template #icon>
        <van-icon :name="item.icon" class="suffix-icon" />
      </template>
    </van-cell>
  </div>
</template>
...
export default {
    data(){
        return {
            userInfo:{
                icon:'https://img.youpin.mi-img.com/shopcenter/415b0fd8150f58ee2b85caa2ee9cef8f.png',
                name:'请登录',
                to:'login'
            },
            focusList:[
